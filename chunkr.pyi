from typing import Any, Dict, List, Optional, Tuple, Union

class Document:
    """A document holding chunked text content and associated metadata."""
    content: str
    metadata: Dict[str, Any]

    def __init__(self, content: str, metadata: Optional[Dict[str, Any]] = None) -> None: ...
    def to_dict(self) -> Dict[str, Any]: ...
    def to_langchain(self) -> Any: ...
    @staticmethod
    def from_langchain(doc: Any) -> Document: ...
    def to_llamaindex(self) -> Any: ...
    @staticmethod
    def from_llamaindex(node: Any) -> Document: ...
    def __len__(self) -> int: ...
    def __repr__(self) -> str: ...

class RecursiveChunker:
    """Splits text recursively along natural semantic boundaries (paragraphs, sentences, words)."""
    def __init__(
        self,
        chunk_size: int = 1000,
        overlap: int = 200,
        separators: Optional[List[str]] = None,
    ) -> None: ...
    def chunk(self, text: str) -> List[Document]: ...
    def chunk_documents(self, docs: List[Document]) -> List[Document]: ...
    def par_chunk_documents(self, docs: List[Document]) -> List[Document]: ...
    def par_chunk_texts(self, texts: List[str]) -> List[List[Document]]: ...

class TokenChunker:
    """Splits text directly by OpenAI BPE token counts (cl100k_base, o200k_base, p50k_base, r50k_base)."""
    def __init__(
        self,
        chunk_size: int = 512,
        overlap: int = 50,
        encoding: str = "cl100k_base",
    ) -> None: ...
    def chunk(self, text: str) -> List[Document]: ...
    def chunk_documents(self, docs: List[Document]) -> List[Document]: ...
    def par_chunk_documents(self, docs: List[Document]) -> List[Document]: ...
    def par_chunk_texts(self, texts: List[str]) -> List[List[Document]]: ...
    def count_tokens(self, text: str) -> int: ...

class HFTokenChunker:
    """Splits text directly by Hugging Face token counts (Llama 3, Mistral, Qwen, BGE, BERT, custom tokenizer.json)."""
    def __init__(
        self,
        json_or_path: str,
        is_file: bool = False,
        chunk_size: int = 512,
        overlap: int = 50,
    ) -> None: ...
    @staticmethod
    def from_file(path: str, chunk_size: int = 512, overlap: int = 50) -> HFTokenChunker: ...
    @staticmethod
    def from_json(json_str: str, chunk_size: int = 512, overlap: int = 50) -> HFTokenChunker: ...
    @staticmethod
    def from_tokenizer(tokenizer: Any, chunk_size: int = 512, overlap: int = 50) -> HFTokenChunker: ...
    def chunk(self, text: str) -> List[Document]: ...
    def count_tokens(self, text: str) -> int: ...
    def chunk_documents(self, docs: List[Document]) -> List[Document]: ...
    def par_chunk_documents(self, docs: List[Document]) -> List[Document]: ...
    def par_chunk_texts(self, texts: List[str]) -> List[List[Document]]: ...

class SentenceChunker:
    """Splits text by sentence boundaries while guarding abbreviations, decimals, and ellipses."""
    def __init__(
        self,
        sentences_per_chunk: int = 3,
        overlap: int = 1,
        max_characters: Optional[int] = None,
    ) -> None: ...
    def chunk(self, text: str) -> List[Document]: ...
    def chunk_documents(self, docs: List[Document]) -> List[Document]: ...
    def par_chunk_documents(self, docs: List[Document]) -> List[Document]: ...
    def par_chunk_texts(self, texts: List[str]) -> List[List[Document]]: ...

class ParagraphChunker:
    """Splits text across paragraph boundaries."""
    def __init__(
        self,
        paragraphs_per_chunk: int = 2,
        overlap: int = 0,
    ) -> None: ...
    def chunk(self, text: str) -> List[Document]: ...
    def chunk_documents(self, docs: List[Document]) -> List[Document]: ...
    def par_chunk_documents(self, docs: List[Document]) -> List[Document]: ...
    def par_chunk_texts(self, texts: List[str]) -> List[List[Document]]: ...

class SemanticChunker:
    """Splits text dynamically where cosine distance between sentence embeddings exceeds statistical thresholds."""
    def __init__(
        self,
        percentile: float = 90.0,
        min_size: int = 100,
        max_size: int = 2000,
    ) -> None: ...
    def chunk(self, text: str) -> List[Document]: ...
    def chunk_documents(self, docs: List[Document]) -> List[Document]: ...
    def par_chunk_documents(self, docs: List[Document]) -> List[Document]: ...
    def par_chunk_texts(self, texts: List[str]) -> List[List[Document]]: ...

class PropositionChunker:
    """Decomposes sentences into atomic, self-contained factual claims (propositions)."""
    def __init__(
        self,
        propositions_per_chunk: int = 1,
        overlap: int = 0,
    ) -> None: ...
    def chunk(self, text: str) -> List[Document]: ...
    def chunk_documents(self, docs: List[Document]) -> List[Document]: ...
    def par_chunk_documents(self, docs: List[Document]) -> List[Document]: ...
    def par_chunk_texts(self, texts: List[str]) -> List[List[Document]]: ...

class ContextualChunker:
    """Enriches chunk contents with document-level context and situational prefaces."""
    def __init__(
        self,
        chunk_size: int = 1000,
        overlap: int = 200,
        max_context_chars: int = 200,
    ) -> None: ...
    def chunk(self, text: str) -> List[Document]: ...
    def chunk_documents(self, docs: List[Document]) -> List[Document]: ...
    def par_chunk_documents(self, docs: List[Document]) -> List[Document]: ...
    def par_chunk_texts(self, texts: List[str]) -> List[List[Document]]: ...

class QueryAwareChunker:
    """Dynamically adapts chunk sizing and boundary placement based on query keyword density."""
    def __init__(
        self,
        query: str,
        hotspot_sentences: int = 2,
        hotspot_overlap: int = 1,
        context_sentences: int = 5,
        relevance_threshold: float = 0.1,
    ) -> None: ...
    def chunk(self, text: str) -> List[Document]: ...
    def chunk_documents(self, docs: List[Document]) -> List[Document]: ...
    def par_chunk_documents(self, docs: List[Document]) -> List[Document]: ...
    def par_chunk_texts(self, texts: List[str]) -> List[List[Document]]: ...

class AgenticChunker:
    """Autonomous agent-based sentence evaluation for discourse transitions and topic shifts."""
    def __init__(
        self,
        min_chars: int = 150,
        max_chars: int = 1200,
    ) -> None: ...
    def chunk(self, text: str) -> List[Document]: ...
    def chunk_documents(self, docs: List[Document]) -> List[Document]: ...
    def par_chunk_documents(self, docs: List[Document]) -> List[Document]: ...
    def par_chunk_texts(self, texts: List[str]) -> List[List[Document]]: ...

class HierarchicalChunker:
    """Multi-tier parent-child hierarchical chunker for Small-to-Big RAG architectures."""
    def __init__(
        self,
        parent_size: int = 2000,
        parent_overlap: int = 200,
        child_size: int = 400,
        child_overlap: int = 50,
        include_parents: bool = True,
    ) -> None: ...
    def chunk(self, text: str) -> List[Document]: ...
    def chunk_hierarchical(self, text: str) -> List[Dict[str, Any]]: ...
    def chunk_tree(self, text: str) -> Dict[str, Any]: ...
    def chunk_documents(self, docs: List[Document]) -> List[Document]: ...
    def par_chunk_documents(self, docs: List[Document]) -> List[Document]: ...
    def par_chunk_texts(self, texts: List[str]) -> List[List[Document]]: ...

class MarkdownChunker:
    """Structure-aware Markdown chunker with header hierarchy parsing and breadcrumb tracking."""
    def __init__(
        self,
        chunk_size: int = 1000,
        overlap: int = 150,
        include_header_in_content: bool = True,
    ) -> None: ...
    def chunk(self, text: str) -> List[Document]: ...
    def chunk_documents(self, docs: List[Document]) -> List[Document]: ...
    def par_chunk_documents(self, docs: List[Document]) -> List[Document]: ...
    def par_chunk_texts(self, texts: List[str]) -> List[List[Document]]: ...

class CodeChunker:
    """Language-aware syntax chunker for Rust, Python, JavaScript, TypeScript, Go, C++, SQL, and Markdown."""
    def __init__(
        self,
        language: str = "rust",
        chunk_size: int = 1500,
        overlap: int = 200,
    ) -> None: ...
    def chunk(self, text: str) -> List[Document]: ...
    def chunk_documents(self, docs: List[Document]) -> List[Document]: ...
    def par_chunk_documents(self, docs: List[Document]) -> List[Document]: ...
    def par_chunk_texts(self, texts: List[str]) -> List[List[Document]]: ...

class AstCodeChunker:
    """Tree-sitter AST syntax chunker for Rust and Python, splitting strictly on syntax boundaries."""
    def __init__(
        self,
        language: str = "rust",
        max_chunk_size: int = 1500,
    ) -> None: ...
    def chunk(self, text: str) -> List[Document]: ...
    def chunk_documents(self, docs: List[Document]) -> List[Document]: ...
    def par_chunk_documents(self, docs: List[Document]) -> List[Document]: ...
    def par_chunk_texts(self, texts: List[str]) -> List[List[Document]]: ...

class JsonChunker:
    """Structure-aware JSON chunker preserving JSON paths and valid sub-objects."""
    def __init__(self, max_size: int = 1500) -> None: ...
    def chunk(self, text: str) -> List[Document]: ...
    def chunk_documents(self, docs: List[Document]) -> List[Document]: ...
    def par_chunk_documents(self, docs: List[Document]) -> List[Document]: ...
    def par_chunk_texts(self, texts: List[str]) -> List[List[Document]]: ...

class HtmlChunker:
    """Structure-aware HTML chunker splitting across semantic DOM elements."""
    def __init__(
        self,
        chunk_size: int = 1200,
        overlap: int = 150,
    ) -> None: ...
    def chunk(self, text: str) -> List[Document]: ...
    def chunk_documents(self, docs: List[Document]) -> List[Document]: ...
    def par_chunk_documents(self, docs: List[Document]) -> List[Document]: ...
    def par_chunk_texts(self, texts: List[str]) -> List[List[Document]]: ...

class TableChunker:
    """Table-aware chunker preserving headers and table schema across Markdown tables, CSV, and TSV."""
    def __init__(
        self,
        chunk_size: int = 1000,
        rows_per_chunk: Optional[int] = None,
        overlap_rows: int = 1,
        format: str = "auto",
    ) -> None: ...
    def chunk(self, text: str) -> List[Document]: ...
    def chunk_documents(self, docs: List[Document]) -> List[Document]: ...
    def par_chunk_documents(self, docs: List[Document]) -> List[Document]: ...
    def par_chunk_texts(self, texts: List[str]) -> List[List[Document]]: ...

class LateChunker:
    """Late chunker computing exact token span boundaries and pooled embeddings from full-document representations."""
    def __init__(
        self,
        encoding: str = "cl100k_base",
        chunk_size: int = 500,
        overlap: int = 50,
        normalize: bool = True,
    ) -> None: ...
    def chunk(self, text: str) -> List[Document]: ...
    def chunk_spans(self, text: str) -> List[Tuple[Document, Tuple[int, int]]]: ...
    def pool_span(self, token_embeddings: List[List[float]], start: int, end: int) -> List[float]: ...
    def pool_embeddings(self, token_embeddings: List[List[float]], chunks: List[Document]) -> List[List[float]]: ...
    def pool_spans(self, token_embeddings: List[List[float]], spans: List[Tuple[int, int]]) -> List[List[float]]: ...
    def chunk_documents(self, docs: List[Document]) -> List[Document]: ...
    def par_chunk_documents(self, docs: List[Document]) -> List[Document]: ...
    def par_chunk_texts(self, texts: List[str]) -> List[List[Document]]: ...

class CharacterChunker:
    """Fixed-size character chunker with configurable overlap."""
    def __init__(
        self,
        chunk_size: int = 1000,
        overlap: int = 200,
    ) -> None: ...
    def chunk(self, text: str) -> List[Document]: ...
    def chunk_documents(self, docs: List[Document]) -> List[Document]: ...
    def par_chunk_documents(self, docs: List[Document]) -> List[Document]: ...
    def par_chunk_texts(self, texts: List[str]) -> List[List[Document]]: ...

class WordChunker:
    """Word-count chunker with configurable overlap."""
    def __init__(
        self,
        chunk_size: int = 200,
        overlap: int = 20,
    ) -> None: ...
    def chunk(self, text: str) -> List[Document]: ...
    def chunk_documents(self, docs: List[Document]) -> List[Document]: ...
    def par_chunk_documents(self, docs: List[Document]) -> List[Document]: ...
    def par_chunk_texts(self, texts: List[str]) -> List[List[Document]]: ...

class PDFLoader:
    """High-performance PDF loader extracting raw text, single documents, or page-by-page chunk collections."""
    def __init__(self) -> None: ...
    def load(self, path: str) -> str: ...
    def load_from_file(self, path: str) -> str: ...
    def load_from_bytes(self, bytes: bytes) -> str: ...
    def load_document(self, path: str) -> Document: ...
    def load_document_from_bytes(self, bytes: bytes) -> Document: ...
    def load_pages(self, path: str) -> List[Document]: ...
    def load_pages_from_file(self, path: str) -> List[Document]: ...
    def load_pages_from_bytes(self, bytes: bytes) -> List[Document]: ...

class DirectoryLoader:
    """Multi-threaded directory loader with automatic chunker routing by file extension."""
    def __init__(
        self,
        recursive: bool = True,
        extensions: Optional[List[str]] = None,
        excludes: Optional[List[str]] = None,
        chunk_size: int = 1000,
        overlap: int = 150,
    ) -> None: ...
    def load(self, path: str) -> List[Document]: ...
    def load_and_chunk(self, path: str) -> List[Document]: ...

class ChunkPacker:
    """Post-processing optimizer that greedily bin-packs small chunks into larger chunks up to a character limit."""
    def __init__(
        self,
        max_characters: int = 1000,
        separator: str = "\n\n",
    ) -> None: ...
    def pack(self, docs: List[Document]) -> List[Document]: ...

class ChunkPipeline:
    """Composable post-chunking transformation pipeline for filtering, deduplication, packing, and enrichment."""
    def __init__(self) -> None: ...
    def filter_min_chars(self, min_chars: int) -> ChunkPipeline: ...
    def filter_max_chars(self, max_chars: int) -> ChunkPipeline: ...
    def filter_min_words(self, min_words: int) -> ChunkPipeline: ...
    def filter_min_alpha_ratio(self, ratio: float) -> ChunkPipeline: ...
    def deduplicate(self, exact: bool = True, case_sensitive: bool = True) -> ChunkPipeline: ...
    def pack(self, max_characters: int) -> ChunkPipeline: ...
    def enrich(self, id_prefix: Optional[str] = None) -> ChunkPipeline: ...
    def process(self, docs: List[Document]) -> List[Document]: ...
    def par_process(self, docs: List[Document]) -> List[Document]: ...

class StreamChunker:
    """Streaming chunker that processes large files or lines with constant memory footprint."""
    def __init__(self, chunk_size: int = 1000, overlap: int = 150) -> None: ...
    def chunk_file(self, path: str) -> List[Document]: ...
    def chunk_text(self, text: str) -> List[Document]: ...
    def chunk_lines(self, lines: List[str]) -> List[Document]: ...

def to_langchain(docs: List[Document]) -> List[Any]: ...
def from_langchain(docs: List[Any]) -> List[Document]: ...
def to_llamaindex(docs: List[Document]) -> List[Any]: ...
def from_llamaindex(nodes: List[Any]) -> List[Document]: ...
def to_dict_list(docs: List[Document]) -> List[Dict[str, Any]]: ...

def load_pdf(path: str) -> str: ...
def load_pdf_pages(path: str) -> List[Document]: ...
